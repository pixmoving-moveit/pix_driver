...
Steeringreport18f::Steeringreport18f() {}
int32_t Steeringreport18f::ID = 0x18F;

void Steeringreport18f::update_bytes(uint8_t bytes_data[8])
{
  for(uint i=0;i<8;i++)
  {
...
double Steeringreport18f::realcurrent() {
  Byte t0(*(bytes + 4));
...
int Steeringreport18f::realangle() {
  Byte t0(*(bytes + 2));
  int32_t x = t0.get_byte(0, 8);
...
int Steeringreport18f::ECUtemputer() {
  Byte t0(*(bytes + 6));
  int32_t x = t0.get_byte(0, 8);
...
bool Steeringreport18f::ECUstate() {
  Byte t0(*(bytes + 0));
